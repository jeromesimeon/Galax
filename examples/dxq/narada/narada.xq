declare server Self implements S at $local:self; 
...
declare variable $local:T_m := 10.0;  

(: Neighbors :)
declare variable $local:neighbors := <neighbors/>; 
declare variable $local:neighbors_mutex :=  dxq:mcreate(); 

(: Members :)
declare variable $local:members := <members/>;
declare variable $local:members_mutex :=  dxq:mcreate(); 

(: Routing info :)
declare variable $local:routes := <routes/>;
...
declare updating function N:init() { 
  ...
  let $fst_neighbors := fn:doc("latencies.xml")/config/graph[@name="Neighbors"]/edge[@s=$local:self] return 
...
(:        insert node $nbrs into $local:neighbors; :)
...
(: fn:trace((), fn:string-join(($local:self,"thinks",$n/@address,"is dead",fn:string($diff),"after",fn:string($t - $local:start))," ")); :)
    N:deadNeighbor($n)  
...
{
  dxq:lock($local:routes_mutex);
    let $route_to_neighbor := $local:routes/route[@address = $neighbor],
        $lat := if (fn:empty($route_to_neighbor)) then N:latencyTo($neighbor)
                else $route_to_neighbor/@latency 
    return
    ...
    return 
      ...
      return 
        let $fst_hop :=  $neighbor_route/hop[1], 
            $to_attr := if (fn:empty($fst_hop)) then () 
                         else attribute to { $fst_hop/@from },
            $new_route :=  
           <route address="{$neighbor_route/@address}" 
                   distance="{$neighbor_route/@distance + 1}"
                   latency="{$lat + $neighbor_route/@latency}">{
              (<hop from="{$neighbor}">{$to_attr}</hop>,$neighbor_route/hop)
            }</route>
        return
        if (fn:empty($route)) then {
          insert node $new_route into $local:routes;
    ...
    if ($memct > 0) then 
     (: First member is always self, so randomly select member after
      ...
      return 
        let server T implement S at $y/@address 
        return  {
(:          dxq:gui-report(fn:string-join(("T:pingMsg@",$y/@address),""));:)
...
declare updating function N:pongMsg($a,$t1) { 
  if ($local:status = "alive") then  
    ...
    return {
      ...
      dxq:lock($local:routes_mutex);
        for $route in 
          $local:routes/route[@address= $a][@latency cast as xs:double > 
                                            $latency cast as xs:double ] 
        return 
          (: Cheaper to talk to this member directly than to go 
             through established route :)
...
declare updating function N:neighborLoop($delay) {
  dxq:gui-report("neighborLoop");
  while (true()) return {
    if ($local:status = "alive") then {
(:  dxq:gui-report($local:self, "N:neighborProbe"); :)
      N:neighborProbe()
    }
    else ();
    dxq:delay($delay)
...
(:  dxq:delay(2);
  N:reportNeighborGraph(); 
:)
  (: Here, we are using an asynchronous call to Self to get concurrent threads :)
...
(: Colors for graph drawing :)
declare variable $local:green := 2150464;
declare variable $local:blue := 2119873;
declare variable $local:yellow := 15464765;

(: BUG! call to N:graphEdge in N:reportRoutingGraph() passing $w = () :)
(: declare function N:graphEdge($s,$t,$l,$w as xs:double) {  :)
declare function N:graphEdge($s,$t,$l,$w as xs:double,$c) { 
  <edge s="{$s}" t="{$t}" l="{$l}" w="{$w}" c="{$c}"/> (: xmlns=""  :)
};

declare updating function N:reportNeighborGraph() {
  let $graph := 
    <graph name="Neighbors">{ (: xmlns=""  :)
      for $n in $local:neighbors/neighbor return 
      let $l := $local:members/member[@address = $n/@address]/latency return
        N:graphEdge($local:self,$n/@address,$l,2,$local:yellow)
    }</graph>
  return 
...
declare updating function N:reportRoutingGraph() {
  let $edges := 
    ...
    return {
      if ($wt > 0) then N:graphEdge($f,$t,"",$wt*3,$local:green) else ()
    }
  return

  let $neighbor_edges := 
    (for $n in $local:neighbors/neighbor return (: Putting this for & next let in one FLWOR raises physical type error :)
    let $nedges := fn:max($edges[@s = $n/@address]/@w) return
      N:graphEdge($local:self,$n/@address,"", (if(fn:empty($nedges)) then 0 else $nedges) + 3, $local:green))
  return

  let $graph := 
    <graph name="Routes">{  (: xmlns=""  :)
