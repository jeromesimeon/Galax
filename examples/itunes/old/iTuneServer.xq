...
declare updating function iTuneServer:addDuration($album as xs:string, $song as xs:string) {
  let $a := iTuneServer:getAlbum($album) return
...
declare updating function iTuneServer:f1() {
  let $x := dxq:cvcreate() return $x
  (dxq:wait(), iTuneServer:getAlbum("Dirty Paper Cup"))
};

declare updating function iTuneServer:f2($x) {
  iTuneServer:addDuration("Dirty Paper Cup","Younger Longer"),
  dxq:broadcast($x)
};
